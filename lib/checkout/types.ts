...
export const CheckoutRequestSchema = z.object({
  ...
  /**
   * UUID v4 generated by the client per submit attempt. Used to make
   * `/api/checkout` idempotent: a double-click or retry returns the
   * same `{clientSecret, orderId}` instead of creating a second Woo
   * order. Optional for backwards compatibility with stale browser
   * tabs that pre-date this field; when absent, the route falls back
   * to non-idempotent behaviour.
   */
  idempotencyKey: z
    .string()
    .regex(
